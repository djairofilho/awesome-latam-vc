---
{
  "schema_version": "1.0",
  "id": "fund:leap-caja-los-andes:es",
  "entity_id": "fund:leap-caja-los-andes",
  "slug": "leap-caja-los-andes",
  "name": "Leap",
  "entity_type": "fund",
  "locale": "es",
  "translation_of": "fund:leap-caja-los-andes:en",
  "translation_status": "complete",
  "summary": "Leap es la plataforma chilena de corporate venture capital de Caja Los Andes para startups con propósito social.",
  "aliases": [
    "Leap by Caja Los Andes"
  ],
  "operator": "Caja Los Andes",
  "base_geography": {
    "kind": "country",
    "code": "CL"
  },
  "countries_covered": [
    "CL"
  ],
  "stages": [
    "not_disclosed"
  ],
  "focuses": [
    "social_impact",
    "fintech",
    "insurtech",
    "wellness",
    "security"
  ],
  "official_website": "https://leap.cajalosandes.cl/",
  "founder_route": null,
  "sources": [
    {
      "title": "Leap corporate venture capital and portfolio",
      "url": "https://leap.cajalosandes.cl/",
      "kind": "official_portfolio"
    },
    {
      "title": "Chile corporate venture activity in 2026",
      "url": "https://fch.cl/noticias/deben-los-corporativos-jugar-bajo-las-reglas-del-venture-capital/",
      "kind": "secondary"
    }
  ],
  "last_verified": "2026-07-30",
  "protected_terms": [
    "Leap",
    "Leap by Caja Los Andes",
    "Caja Los Andes"
  ]
}
---
# Leap

Leap es la plataforma chilena de corporate venture capital de Caja Los Andes para startups con propósito social.

## Perfil de inversión

- **Sitio web:** https://leap.cajalosandes.cl/
- **Tipo de fondo:** Corporate venture capital
- **También conocido como:** Leap by Caja Los Andes
- **Inversión directa en startups:** Sí
- **Abierto a founders externos:** No divulgado públicamente
- **Etapa de entrada:** Not publicly disclosed
- **Etapas posteriores:** No divulgado públicamente
- **Foco:** Social impact, fintech, insurtech, wellness, and security
- **Geografía:** Chile
- **Cheque inicial:** Not publicly disclosed
- **Papel en la inversión:** Inversionista directo
- **Modelos de negocio:** No divulgado públicamente
- **Tamaño del portafolio:** 3 inversiones directas divulgadas
- **Empresas seleccionadas:** Betterfly, Soy Focus y Soyio
- **Actividad actual:** Current direct-investment portfolio verified in 2026
- **Enviar una startup:** No divulgado públicamente

## Tesis declarada

Las fuentes oficiales confirman la tesis de Leap y su modelo recurrente de inversión directa, sin ampliar los criterios más allá de lo publicado.

## Señales del portafolio

La señal observada registrada en la auditoría es: Current direct-investment portfolio verified in 2026. Esta señal no amplía la tesis declarada.

## Fuentes

- [Leap corporate venture capital and portfolio](https://leap.cajalosandes.cl/)
- [Chile corporate venture activity in 2026](https://fch.cl/noticias/deben-los-corporativos-jugar-bajo-las-reglas-del-venture-capital/)

**Última verificación:** 2026-07-30
