"""Consolida de forma determinista los shards de la auditoría mexicana."""
...
def main() -> None:
    grouped: dict[str, list[dict]] = {filename: [] for filename, _ in KINDS.values()}
    seen: set[str] = set()
    for shard in sorted((ROOT / "shards").glob("worker-*/records.jsonl")):
        for number, line in enumerate(shard.read_text(encoding="utf-8").splitlines(), 1):
            ...
            primary_key = next(
                (key for key in ("evidence_id", "call_id", "program_id", "agency_id") if key in record),
                None,
            )
            if primary_key is None:
                raise ValueError(f"{shard}:{number}: tipo de registro ambiguo")
