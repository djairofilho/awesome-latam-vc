# Reauditoria de fundos — Uruguai

Data de corte: `2026-07-30`. Esta é uma cobertura auditada das fontes percorridas, sem alegação de totalidade.

- baseline congelado: 203 perfis;
- 34 fontes não regulatórias, todas com owner e estado terminal;
- 36 candidatos: 5 elegíveis, 13 duplicatas, 14 encaminhados e 4 com evidência insuficiente;
- 100% das origens de descoberta são não regulatórias;
- 2 consultas pontuais ao Banco Central del Uruguay (5.6%), apenas para identidade/divergência;
- busca cega em duas famílias novas e passagem final de saturação com rendimento marginal zero;
- amostra determinística de 4/17 exclusões: uy-ewa-capital, uy-gridx, uy-draco-capital, uy-prosperitas-capital-partners;
- revisão independente de #287 aprovada por `integrator`, sem achados críticos ou altos;
- lote congelado com 5 fundos e 15 perfis completos em inglês, português e espanhol.
