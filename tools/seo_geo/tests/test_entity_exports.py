...
class EntityExportTests(unittest.TestCase):
    ...
    def test_all_229_profiles_are_exported_once_in_stable_order(self):
        entities = self.document["entities"]
        ids = [entity["id"] for entity in entities]
        profile_ids = sorted(
            profile.metadata["entity_id"] for profile in self.profiles
        )
        self.assertEqual(229, len(entities))
        self.assertEqual(profile_ids, ids)
        self.assertEqual(ids, sorted(ids))
        self.assertEqual(len(ids), len(set(ids)))
        self.assertEqual(229, self.document["dataset"]["entity_count"])
