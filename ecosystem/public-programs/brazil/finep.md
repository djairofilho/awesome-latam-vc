---
{
  "schema_version": "1.0",
  "id": "public_program:finep:en",
  "entity_id": "public_program:finep",
  "slug": "finep",
  "name": "Financiadora de Estudos e Projetos",
  "entity_type": "public_program",
  "locale": "en",
  "translation_of": null,
  "translation_status": "canonical",
  "summary": "A Finep mantém uma rota recorrente para startups lideradas por mulheres, com premiação financeira e sete edições até 2026.",
  "aliases": [
    "Finep"
  ],
  "operator": null,
  "base_geography": {
    "kind": "country",
    "code": "BR"
  },
  "countries_covered": [
    "BR"
  ],
  "stages": [
    "not_applicable"
  ],
  "focuses": [
    "entrepreneurship",
    "innovation"
  ],
  "official_website": "https://www.finep.gov.br/",
  "founder_route": "https://www.finep.gov.br/chamadas-publicas/chamadapublica/781",
  "sources": [
    {
      "title": "Prêmio Mulheres Inovadoras 7ª Edição — 2026",
      "url": "https://www.finep.gov.br/chamadas-publicas/chamadapublica/781",
      "kind": "official_program"
    }
  ],
  "last_verified": "2026-07-27",
  "protected_terms": [
    "Financiadora de Estudos e Projetos"
  ]
}
---
# Financiadora de Estudos e Projetos

- **Entity ID:** `agency-finep`
- **Entity type:** Public agency
- **Aliases:** Finep
- **Website:** https://www.finep.gov.br/
- **Geography:** Brasil
- **Founder route:** https://www.finep.gov.br/chamadas-publicas/chamadapublica/781
- **Publication batch:** `public-programs-01`

A Finep mantém uma rota recorrente para startups lideradas por mulheres, com premiação financeira e sete edições até 2026.

This agency profile represents the stable public route. Program-specific benefits, eligibility, values, dates, and availability remain attached to each program or call.

## Published programs

- [Programa Mulheres Inovadoras](finep-mulheres-inovadoras.md) (`program-finep-mulheres-inovadoras`)

## Official sources

- [Prêmio Mulheres Inovadoras 7ª Edição — 2026](https://www.finep.gov.br/chamadas-publicas/chamadapublica/781) — A Finep registra a sétima edição do programa para startups lideradas por mulheres. (`evidence-agency-finep`)

**Last verified:** 2026-07-27
